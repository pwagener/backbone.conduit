/**
 * This test module ensures the QuickStart collection is backwards compatible
 * with a base Collection.
 */
define([
    'backbone',
    "../lib/QuickStartCollection"
], function(Backbone, QuickStartCollection) {

    var a, b, c, d, e, col, otherCol;

    module("QuickStartCollection Compatibility", {
        ...
        setup: function() {
            ...
            e         = null;
            col       = new QuickStartCollection([a,b,c,d]);
            otherCol  = new QuickStartCollection();
        }
    });
    ...
    test("String comparator.", 1, function() {
        var collection = new QuickStartCollection([
            {id: 3},
            {id: 1},
            {id: 2}
        ], {comparator: 'id'});
    ...
    test("new and parse", 3, function() {
        var Collection = QuickStartCollection.extend({
            parse : function(data) {
    ...
    test("get with non-default ids", 5, function() {
        var col = new QuickStartCollection();
        ...
        var col2 = new QuickStartCollection();
    ...
    test('get with "undefined" id', function() {
        var collection = new QuickStartCollection([{id: 1}, {id: 'undefined'}]);
    ...
    test("add", 14, function() {
        ...
        var atCol = new QuickStartCollection([f, g, h]);
        ...
        var coll = new QuickStartCollection(new Array(2));
        var addCount = 0;
    ...
    test("add multiple models", 6, function() {
        var col = new QuickStartCollection([{at: 0}, {at: 1}, {at: 9}]);
    ...
    test("add; at should have preference over comparator", 1, function() {
        var Col = QuickStartCollection.extend({
    ...
    test("can't add model to collection twice", function() {
        var col = new QuickStartCollection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
    ...
    test("add model to multiple collections", 10, function() {
        ...
        var colE = new QuickStartCollection([]);
        colE.on('add', function(model, collection) {
            equal(e, model);
            equal(colE, collection);
        });
        var colF = new QuickStartCollection([]);
    ...
    test("add model with parse", 1, function() {
        ...
        var Col = QuickStartCollection.extend({model: Model});
        var col = new Col;
    ...
    test("shift and pop", 2, function() {
        var col = new QuickStartCollection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
    ...
    test("events are unbound on remove", 3, function() {
        ...
        var emcees = new QuickStartCollection([dj]);
    ...
    test("remove in multiple collections", 7, function() {
        ...
        var colE = new QuickStartCollection([e]);
        var colF = new QuickStartCollection([f]);
        ok(e != f);
        ok(colE.length === 1);
        ok(colF.length === 1);
        colE.remove(e);
    ...
    test("remove same model in multiple collection", 16, function() {
        ...
        var colF = new QuickStartCollection([e]);
    ...
    test("create with validate:true enforces validation", 3, function() {
        ...
        var ValidatingCollection = QuickStartCollection.extend({
    ...
    test("where and findWhere", 8, function() {
        ...
        var coll = new QuickStartCollection([
            model,
            {a: 1},
            {a: 1, b: 2},
            {a: 2, b: 2},
    ...
    test ("reset with different values", function(){
        var col = new QuickStartCollection({id: 1});
        col.reset({id: 1, a: 1});
    ...
    test("same references in reset", function() {
        ...
        var collection = new QuickStartCollection({id: 1});
    ...
    test("reset passes caller options", 3, function() {
        ...
        var col = new (QuickStartCollection.extend({ model: Model }))();
    ...
    test("add does not alter arguments", 2, function(){
        var attrs = {};
        var models = [attrs];
        new QuickStartCollection().add(models);
    ...
    test("#964 - collection.get return inconsistent", 2, function() {
        var c = new QuickStartCollection();
    ...
    test("#1112 - passing options.model sets collection.model", 2, function() {
        var Model = Backbone.Model.extend({});
        var c = new QuickStartCollection([{id: 1}], {model: Model});
        ok(c.model === Model);
    ...
    test("null and undefined are invalid ids.", 2, function() {
        var model = new Backbone.Model({id: 1});
        var collection = new QuickStartCollection([model]);
        model.set({id: null});
    ...
    test("#1655 - groupBy can be used with a string argument.", 3, function() {
        var collection = new QuickStartCollection([{x: 1}, {x: 2}]);
    ...
    test("#1655 - sortBy can be used with a string argument.", 1, function() {
        var collection = new QuickStartCollection([{x: 3}, {x: 1}, {x: 2}]);
    ...
    test("`sort` shouldn't always fire on `add`", 1, function() {
        var c = new QuickStartCollection([{id: 1}, {id: 2}, {id: 3}], {
            comparator: 'id'
        });
    ...
    test("set", function() {
        ...
        var c = new QuickStartCollection([m1, m2]);
    ...
    test("set + merge with default values defined", function() {
        ...
        var col = new QuickStartCollection([m], {model: Model});
    ...
    test('merge without mutation', function () {
        ...
        var Collection = QuickStartCollection.extend({model: Model});
    ...
    test('`set` matches input order in the absence of a comparator', function () {
        ...
        var collection = new QuickStartCollection([one, two, three]);
    ...
    test("#1915 - `parse` data in the right order in `set`", function() {
        var collection = new (QuickStartCollection.extend({
            parse: function (data) {
    ...
    test("#2612 - nested `parse` works with `Collection#set`", function() {
        ...
        var Item = Backbone.Model.extend({
            constructor: function() {
                this.subItems = new QuickStartCollection();
                Backbone.Model.apply(this, arguments);
            },
            parse: function(attrs) {
        ...
        var Items = QuickStartCollection.extend({
            model: Item
        });
    ...
    test('_addReference binds all collection events & adds to the lookup hashes', 9, function() {
        ...
        var Collection = QuickStartCollection.extend({
            ...
            _addReference: function(model) {
                QuickStartCollection.prototype._addReference.apply(this, arguments);
                calls.add++;
            ...
            _removeReference: function(model) {
                QuickStartCollection.prototype._removeReference.apply(this, arguments);
                calls.remove++;
